...
def simulate_and_log(m1i, long_sig, short_sig, start_utc, end_utc, *, lookahead=150, tp_R=3.0,
    ...
    sig = sig & (sig.index >= start_utc) & (sig.index <= end_utc)
    idx = m1i.index[sig]
    ...
    for ts in idx:
        ...
        fw = m1i.loc[(m1i.index > ts) & (m1i.index <= end_utc)].iloc[:lookahead]
...
def main():
    ...
    # Signals (good quick config from earlier run)
    ...
    out_csv = "logs/trades_2025-09-08_2025-09-12.csv"
    df.to_csv(out_csv, index=False)

    # Print all trades
    print("\n=== TRADES (2025-09-08 → 2025-09-12, UTC) ===")
    ...
    total_usd = final_equity - 100.0
    print("\n=== SUMMARY ===")
    print(f"Trades: {len(rows)}  Wins: {wins}  Losses: {losses}  BE/Timeout: {be_to}")
    print(f"Win rate: {winrate:.2f}%  Total PnL: {total_R:.1f} R  (${total_usd:.2f})")
    print(f"Final equity (start $100, compounding, risk {risk_pct*100:.1f}%): ${final_equity:.2f}")
