...
# ---------------- OB/OS indicators (no deprecated fillna) ----------------
def rsi(close, n=14):
    delta = close.diff()
...
def cool_down_filter(index, minutes=20, per_hour_cap=3):
    # Enforce min distance between entries and a cap per rolling hour
...
# ---------------- Build signals (existing + stricter OB/OS) ----------------
def build_signals_v2(h1i, m15i, m1i, *, warmup=300, adx_min=18, vol_mult=1.05, don_len=20, supertrend_req=True):
    h1_up_m1, h1_down_m1, bos_up_m1, bos_down_m1 = topdown_masks(h1i, m15i, m1i, adx_min=adx_min)

    # Momentum + breakouts (same as before)
    ...
    # --- OB/OS v2 (stricter): require 4/6 flags + anchor to M15 EMA21/Bands + candle confirm + distance/mean reversion
    ...
    # Anchor to M15 context (EMA21 and Bollinger)
    ...
    # Distance constraints: avoid chasing far outside mean; prefer reversion zone ±0.5*ATR15 around EMA21 or touches outer band
    ...
    # Stricter score >= 4 and must be anchored + confirmed
...
# ---------------- Simulation with sessions, cooldown, BE at +1R ----------------
def session_mask_index(index_utc):
    # London morning + NY overlap (Lagos time)
    windows = [("07:00","11:30"), ("13:30","17:00")]
    return session_mask(index_utc, windows)
...
def simulate_window(start_equity=100.0, risk_pct=None):
    ...
    print(f"Symbol={sym}  Window={start.date()} → {end.date()}  (OB/OS v2, BE@1R, sessions, cooldown)")
    ...
    # Cool-down and per-hour cap
    allowed = cool_down_filter(raw_idx, minutes=20, per_hour_cap=3)
    sig_idx = raw_idx[allowed.loc[raw_idx].values]

    if len(sig_idx)==0:
        print("No signals found in this window after filters.")
        return

    if risk_pct is None:
        risk_pct = float(os.getenv("RISK_PCT", "0.02"))  # default 2%
    equity = float(start_equity)

    rows=[]
    wins=losses=be_to=0
    total_R=0.0
    lookahead=180  # a bit longer since entries are fewer
    ...
    for ts in sig_idx:
        ...
        if long:
            sl = entry - 1.5*atr
            r1 = entry + (entry - sl)               # +1R
            tp = entry + 3.0*(entry - sl)           # +3R
        ...
        # BE at +1R: if 1R first -> cannot lose (0R), else first-hit TP/SL
    ...
    out_csv = 'logs/trades_obos_v2_2025-09-01_2025-09-12.csv'
