...
def simulate_usd(m1i, long_sig, short_sig, start_utc, *, lookahead=150, tp_R=3.0,
    ...
    for ts in idx:
        ...
        if move_BE_at_1R and hit_1R.any():
            if hit_tp.any(): res_R = tp_R; wins += 1
            else: be_to += 1
        else:
            if hit_tp.any() and hit_sl.any():
                t_tp = fw.index[hit_tp.argmax()]; t_sl = fw.index[hit_sl.argmax()]
                if t_tp <= t_sl: res_R = tp_R; wins += 1
                else: res_R = -1.0; losses += 1
            elif hit_tp.any(): res_R = tp_R; wins += 1
            elif hit_sl.any(): res_R = -1.0; losses += 1
            else: be_to += 1
    ...
    return {"trades": int(sig.sum()), "wins": wins, "losses": losses, "be_or_to": be_to,
            "winrate": winrate, "total_R": total_R, "final_equity": equity}
...
def main():
    ...
    start = now - timedelta(days=1)      # yesterday → now
    start_ext = start - timedelta(days=7)  # warmup data for indicators

    print(f"Equity report for {sym} | window (UTC): {start} → {now}")
    ...
    print("\n=== SUMMARY (yesterday → now, start $100, compounding) ===")
    print(f"Trades: {res['trades']}  Wins: {res['wins']}  Losses: {res['losses']}  BE/Timeout: {res['be_or_to']}")
    print(f"Win rate: {res['winrate']:.2f}%")
    print(f"Total PnL: {res['total_R']:.1f} R")
    print(f"Final equity: ${res['final_equity']:.2f}  (risk {risk_pct*100:.1f}% per trade)")

    os.makedirs("logs", exist_ok=True)
    with open("logs/equity_yesterday.txt","w") as f:
        f.write(f"{res}\n")
    print("Saved: logs/equity_yesterday.txt")
