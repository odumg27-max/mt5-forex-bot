...
def init_mt5():
    ...
    log.info("MT5 initialized.")                       password=MT5_PASSWORD, server=MT5_SERVER)
    else:
...
if __name__ == "__main__":
    main()
# ===== Fallback indicators (auto-used if indicators.add_indicators is missing) =====
import numpy as _np
import pandas as _pd

def _fallback_add_indicators(df):
    out = df.copy()
    ...
    upmove   = h.diff()
    downmove = -l.diff()
    plusDM  = _pd.Series(_np.where((upmove>downmove) & (upmove>0), upmove, 0.0), index=out.index)
    minusDM = _pd.Series(_np.where((downmove>upmove) & (downmove>0), downmove, 0.0), index=out.index)
    plusDM14  = plusDM.ewm(alpha=1/14, adjust=False).mean()
    minusDM14 = minusDM.ewm(alpha=1/14, adjust=False).mean()
    atr_safe  = atr14.replace(0, _np.nan)
    plusDI  = 100 * (plusDM14 / atr_safe)
    minusDI = 100 * (minusDM14 / atr_safe)
    dx = 100 * (plusDI.subtract(minusDI).abs() / (plusDI.add(minusDI)).replace(0, _np.nan))
    out["adx14"] = dx.ewm(alpha=1/14, adjust=False).mean().fillna(0.0)

    # Volume MA
    out["vol_ma20"] = v.rolling(20).mean().bfill()
    ...
    for i in range(1, len(out)):
        # final upper band
        if _np.isfinite(final_upper.iat[i-1]):
            cond = (upper.iat[i] < final_upper.iat[i-1]) or (c.iat[i-1] > final_upper.iat[i-1])
            final_upper.iat[i] = upper.iat[i] if cond else final_upper.iat[i-1]
        # final lower band
        if _np.isfinite(final_lower.iat[i-1]):
            cond = (lower.iat[i] > final_lower.iat[i-1]) or (c.iat[i-1] < final_lower.iat[i-1])
            final_lower.iat[i] = lower.iat[i] if cond else final_lower.iat[i-1]

    st = _pd.Series(_np.nan, index=out.index)
    st_dir = _pd.Series(1, index=out.index, dtype="int64")
    for i in range(1, len(out)):
        if _np.isfinite(st.iat[i-1]) and st.iat[i-1] == final_upper.iat[i-1]:
            if c.iat[i] <= final_upper.iat[i]:
                st.iat[i] = final_upper.iat[i]; st_dir.iat[i] = -1
            else:
                st.iat[i] = final_lower.iat[i]; st_dir.iat[i] =  1
        elif _np.isfinite(st.iat[i-1]) and st.iat[i-1] == final_lower.iat[i-1]:
            if c.iat[i] >= final_lower.iat[i]:
                st.iat[i] = final_lower.iat[i]; st_dir.iat[i] =  1
            else:
                st.iat[i] = final_upper.iat[i]; st_dir.iat[i] = -1
        else:
            st.iat[i] = final_lower.iat[i] if c.iat[i] >= final_lower.iat[i] else final_upper.iat[i]
            st_dir.iat[i] = 1 if c.iat[i] >= final_lower.iat[i] else -1

    out["st_dir"] = st_dir.astype(int)
    return out

# If indicators.add_indicators is missing, transparently use the fallback.
try:
    import indicators as _ind_mod
    _HAS_ADD = hasattr(_ind_mod, "add_indicators")
except Exception:
    _HAS_ADD = False

if not _HAS_ADD:
    def add_indicators(df):
        return _fallback_add_indicators(df)
# ===== End fallback block =====
