...
# Load .env if python-dotenv is available (so RISK_PCT is picked up)
try:
...
try:
    ...
    # Fallback UTC+1 if zoneinfo missing
...
def main():
    ...
    # Build signals (unchanged thresholds/logic)
    ...
    # ====== PnL in $ with $100 start (compounding), using .env RISK_PCT or default 0.02 (2%) ======
    try:
    ...
    start_equity = 100.0
    equity = start_equity
    lookahead = 150  # minutes (bars) to check TP/SL on M1

    rows = []
    wins=losses=0; pnl_total_usd=0.0; pnl_total_R=0.0; trades=0
    ...
    out_csv = "logs/trades_45d_equity.csv"
    ...
    print("\\n=== SUMMARY ($ PnL with compounding) ===")
    print(f"Trades: {trades}  Wins: {wins}  Losses: {losses}")
    print(f"Risk per trade: {risk_pct*100:.2f}%  (from .env RISK_PCT or default 2%)")
    print(f"Total PnL (R): {pnl_total_R:.1f}")
    print(f"Total PnL ($): ${pnl_total_usd:.2f}")
