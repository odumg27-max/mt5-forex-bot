# equity_yday_now_rr11.py
# Window: Yesterday 00:00 (Africa/Lagos) -> Now
# Strategy unchanged EXCEPT Risk:Reward set to 1:1 (TP = 1R)
import os, numpy as np, pandas as pd, MetaTrader5 as mt5
from datetime import datetime, timedelta, timezone, time
from bot_mt5 import init_mt5, add_indicators, SYMBOL

# --- Config overrides ---
RR_MULT  = 1.0   # <-- 1R take profit (Risk:Reward = 1:1)
SL_ATR   = 1.5   # stop distance = 1.5 * ATR (same as your strategy)
...
def main():
    ...
    for i, ts in enumerate(trade_idx, 1):
        ...
        # --- ATR-based SL and 1:1 RR TP ---
        if side=="buy":
            sl = entry - SL_ATR*atr
            tp = entry + RR_MULT*(entry - sl)          # <-- 1R target
            fw = m1i.loc[ts:].iloc[1:LOOKAHEAD]
            tp_hit = (fw["high"] >= tp); sl_hit = (fw["low"] <= sl)
        else:
            sl = entry + SL_ATR*atr
            tp = entry - RR_MULT*(sl - entry)          # <-- 1R target
            fw = m1i.loc[ts:].iloc[1:LOOKAHEAD]
    ...
    out_csv = "logs/trades_yday_now_rr11.csv"
    ...
    print("\n=== EQUITY REPORT (Yesterday → Now, RR 1:1) ===")
