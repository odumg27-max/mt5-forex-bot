...
def usd_per_lot_for_move(symbol, price_move):
    si = _sym_info(symbol)
    # convert price distance to "ticks" then USD
...
def simulate_window(start_equity=100.0, risk_pct=None):
    ...
    SPREAD_CAP_POINTS = float(os.getenv("SPREAD_CAP_POINTS", "25"))   # points (EURUSD: 25 ≈ 0.25 pip on 5-digit)
    SLIP_POINTS       = float(os.getenv("SLIP_POINTS", "3"))          # entry slippage (points)
    COMM_PER_LOT_USD  = float(os.getenv("COMM_PER_LOT_USD", "7.0"))   # round-turn commission per 1.00 lot
    NEWS_ATR_MULT     = float(os.getenv("NEWS_ATR_MULT", "4.0"))      # reject if bar range > k * ATR

    print(f"Symbol={sym}  Window={start.date()} \u2192 {end.date()}  (OB/OS v2 + realism)")
    ...
    # ensure 'spread' exists (points); fallback to current symbol spread if missing
    si = _sym_info(sym)
    ...
    allowed = cool_down_filter(raw_idx, minutes=20, per_hour_cap=3)
    sig_idx = raw_idx[allowed.loc[raw_idx].values]
    ...
    for ts in sig_idx:
        ...
        # news-spike guard on the signal bar
        if (row["high"] - row["low"]) > NEWS_ATR_MULT * atr:
            continue

        # spread cap at the bar close (points)
        ...
        entry = entry_raw + (slip_px if long else -slip_px)

        # ATR-based SL/TP with 1:3 RR, recomputed from slipped entry
        if long:
            sl = entry - 1.5*atr
            r1 = entry + (entry - sl)
            tp = entry + 3.0*(entry - sl)
            fw = m1i.loc[ts:].iloc[1:lookahead]
            hit_1R = fw["high"]>=r1; hit_tp = fw["high"]>=tp; hit_sl = fw["low"]<=sl
        else:
            sl = entry + 1.5*atr
            r1 = entry - (sl - entry)
            tp = entry - 3.0*(sl - entry)
            fw = m1i.loc[ts:].iloc[1:lookahead]
            hit_1R = fw["low"]<=r1; hit_tp = fw["low"]<=tp; hit_sl = fw["high"]>=sl

        # determine outcome with BE@+1R
        res_R, outcome = 0.0, "timeout"
        if hit_1R.any():
            t1 = fw.index[hit_1R.argmax()]
            if hit_tp.any():
        ...
        # position sizing (risk % of current equity)
        risk_usd = equity * risk_pct
        stop_px  = abs(entry - sl)
        usd_per_lot = usd_per_lot_for_move(sym, stop_px) or 0.0
        if usd_per_lot <= 0: continue
        lots = round_lots(sym, risk_usd / usd_per_lot)
        if lots <= 0: continue

        # PnL with commission (round-turn)
    ...
    out_csv = "logs/trades_obos_v2_2025-01-01_2025-09-12.csv"
    ...
    print(f"SpreadCap: {SPREAD_CAP_POINTS} pts   Slippage: {SLIP_POINTS} pts   Commission: ${COMM_PER_LOT_USD}/lot RT   NewsGuard: {NEWS_ATR_MULT}*ATR")
...
if __name__ == "__main__":
    simulate_window(start_equity=20.0)
