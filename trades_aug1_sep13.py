...
def simulate_and_log(m1i, long_sig, short_sig, start_utc, end_utc, *,
                     lookahead=150, tp_R=3.0, start_equity=100.0, risk_pct=0.02):
    sig = (long_sig | short_sig).copy()
    sig.loc[(sig.index < start_utc) | (sig.index > end_utc)] = False
    idx = m1i.index[sig]
    ...
    return {
        "rows": rows,
        "wins": wins, "losses": losses, "be_to": be_to,
...
def main():
    init_mt5()
    sym = SYMBOL
    risk_pct = float(os.getenv("RISK_PCT","0.02"))  # from .env if set

    now = datetime.now(timezone.utc)
    year = now.year
    start = datetime(year, 8, 1, 0, 0, tzinfo=timezone.utc)
    req_end = datetime(year, 9, 13, 23, 59, tzinfo=timezone.utc)
    end = min(now, req_end)

    # extra warmup for indicators
    start_ext = start - timedelta(days=10)

    print(f"Trades for {sym} | window (UTC): {start} → {end}  (compounding from $100, risk {risk_pct*100:.1f}%)")
    ...
    # Signals (robust high-win quick config)
    ...
    out_csv = "logs/trades_aug1_sep13.csv"
    df.to_csv(out_csv, index=False)

    # Print all trades
    print("\n=== TRADES (Aug 1 → Sep 13, UTC) ===")
    pd.set_option("display.max_rows", None)
    print(df[["i","time_utc","side","entry","sl","tp","atr","R","outcome","risk_usd","pnl_usd","equity_after"]].to_string(index=False))

    # Summary
    print("\n=== SUMMARY ===")
    print(f"Trades: {len(rows)}  Wins: {res['wins']}  Losses: {res['losses']}  BE/Timeout: {res['be_to']}")
    print(f"Win rate: {res['winrate']:.2f}%")
    print(f"Total PnL: {res['total_R']:.1f} R  |  ${res['total_pnl_usd']:.2f}")
    print(f"Final equity (start $100): ${res['final_equity']:.2f}")
