...
def simulate_and_log(m1i, long_sig, short_sig, start_utc, *, lookahead=150, tp_R=3.0,
...
def main():
    ...
    start = now - timedelta(days=1)          # yesterday → now (UTC)
    start_ext = start - timedelta(days=7)    # warmup for indicators

    print(f"Trades for {sym} | window (UTC): {start} → {now}")
    ...
    # Signals (same settings used in quick wins run)
    ...
    out_csv = "logs/trades_yesterday.csv"
    df.to_csv(out_csv, index=False)

    # Print all trades
    print("\n=== TRADES (yesterday → now, UTC) ===")
